...
const resp = (inner: string) =>
  `<Envelope><Body><rkdbResponse><paket_nr>1</paket_nr><ts_erstellung>x</ts_erstellung>${inner}</rkdbResponse></Body></Envelope>`;
const ok = (extra = '') =>
  resp(`<result><satznr>1</satznr><rkdbMessage><rc>0</rc><msg>ok</msg></rkdbMessage>${extra}</result>`);

test('kasse.registriere liefert Ergebnis und sendet registrierung_kasse', async () => {
  let body = '';
  const rksv = rksvMit(ok(), (b) => {
    body = b;
  });
...
test('kasse.meldeAusfall setzt begruendung/beginn', async () => {
  ...
  await rksv.kasse.meldeAusfall({
    ...
    kassenidentifikationsnummer: 'K1',
    begruendung: 5,
    beginn: new Date('2026-07-20T10:00:00Z'),
  });
...
test('kasse.nimmAusserBetrieb setzt ausserbetriebnahme', async () => {
  ...
  await rksv.kasse.nimmAusserBetrieb({ paketNr: 1, kassenidentifikationsnummer: 'K1', begruendung: 6 });
...
test('beleg.pruefe liefert volles Ergebnis mit Prüfungsbaum', async () => {
  const rksv = rksvMit(
    ok(
      '<verificationResultList><verificationResult><verificationId>1</verificationId><version>1</version><verificationName>Struktur</verificationName><verificationState>PASS</verificationState><verificationTimestamp>t</verificationTimestamp></verificationResult></verificationResultList>',
    ),
  );
