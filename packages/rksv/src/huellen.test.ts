...
test('kasse.registriere liefert Ergebnis und sendet registrierung_kasse', async () => {
  ...
  const erg = await rksv.kasse.registriere({ paketNr: 1, kassenidentifikationsnummer: 'K1', benutzerschluessel: 'A'.repeat(44) });
...
test('kasse.meldeAusfall setzt begruendung/beginn', async () => {
  let body = '';
  const rksv = rksvMit(ok(), (b) => { body = b; });
  await rksv.kasse.meldeAusfall({ paketNr: 1, kassenidentifikationsnummer: 'K1', begruendung: 5, beginn: new Date('2026-07-20T10:00:00Z') });
...
test('see.registriere sendet registrierung_se mit vda_id', async () => {
  let body = '';
  const rksv = rksvMit(ok(), (b) => { body = b; });
  await rksv.see.registriere({ paketNr: 1, artSe: 'HSM_DIENSTLEISTER', vdaId: 'AT9', zertifikatsseriennummer: '1a2b' });
