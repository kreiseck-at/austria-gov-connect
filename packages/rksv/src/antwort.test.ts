...
test('ok-Ergebnis: rc 0 -> ok true', () => {
  const root = wrap(
    '<result><satznr>1</satznr><rkdbMessage><rc>0</rc><msg>Aufruf ok</msg></rkdbMessage></result>',
  );
...
test('fachlicher rc B1 -> ok false, rc/msg durchgereicht', () => {
  const root = wrap(
    '<result><satznr>1</satznr><rkdbMessage><rc>B1</rc><msg>bereits registriert</msg></rkdbMessage></result>',
  );
  const erg = parseRkdbErgebnisse(root);
...
test('kundeninfo wird durchgereicht, wenn im result vorhanden', () => {
  const root = wrap(
    '<result><satznr>1</satznr><kundeninfo>REF-1</kundeninfo><rkdbMessage><rc>0</rc><msg>ok</msg></rkdbMessage></result>',
  );
  const erg = parseRkdbErgebnisse(root);
